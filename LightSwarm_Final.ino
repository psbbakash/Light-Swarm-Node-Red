...
//For LED Bar Graph
int ledcount = 10;
int ledpins[] = {5, 4, 0, 14, 12, 16, 15, 3, 1, 10};  
...
*/
  ...
  LDR_Reading = swarmClear[mySwarmID];
...
/*  uint16_t r, g, b, c, colorTemp, lux;
  ...
  pwm = map(LDR_Reading, 0,1024,0,255);
  analogWrite(Blink_LED,pwm);
    
  int ledlevel = map(LDR_Reading, 0, 1023, 0, ledcount);
  Serial.print("LED Bar Graph Mapped Level = ");
...
void sendLightUpdatePacket(IPAddress & address)
{
  ...
  packetBuffer[5] = (LDR_Reading & 0xFF00) >> 8; // Clear High Byte
  packetBuffer[6] = (LDR_Reading & 0x00FF); // Clear Low Byte
